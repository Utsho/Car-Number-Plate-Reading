...
for root,d,files in os.walk(p1):
    for filename in files:
        ...
        '''image=cv2.imread("step1_output/"+filename,0)
        	...
        	for j in range(0,col):
        		...
        		for l in range(i-3,i+3):
        			for k in range(j-3,j+3):
        				if (l>=0 and l<row and k>=0 and k<col):
        					v=image.item(l,k)
        					if v>maximum:
        						maximum=v
        					if v<minimum:
        ...
        image=imread("step1_output/"+filename,as_grey=True)
        #image = imread("step1_output/"+filename, as_grey=True)
        ...
        plt.imsave('angle/'+filename, sinogram, cmap = plt.cm.gray)
